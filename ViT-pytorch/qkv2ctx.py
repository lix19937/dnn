...
#
    ...
    def transpose_for_scores(self, x): # x shape[1, 512, 256]
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(*new_x_shape) # [1, 512, 8, 32]
        return x.permute(0, 2, 1, 3) # [1, 8, 512, 32]
    ...
    ## optimize of `forward_vit`, just scale in ahead of permute  
    def forward_scale_in_ahead(self, query, key, value):
        assert query.dim() == 3, "only support batch_first"
        mixed_query_layer = self.query(query)
        mixed_key_layer = self.key(key)
        mixed_value_layer = self.value(value)
    ...
    def forward(self, query, key, value):
      return self.forward_scale_in_ahead(query, key, value)
...
#       :math:`E_q` is the query embedding dimension ``embed_dim``.
    ...
    model.eval()    
    run_model(dummy_input_q.permute(1, 0, 2), dummy_input_k.permute(1, 0, 2), dummy_input_v.permute(1, 0, 2), model, "./output/vit_self_atten_v1.outdata")
    torch.onnx.export(model,
                      (dummy_input_q.permute(1, 0, 2), dummy_input_k.permute(1, 0, 2), dummy_input_v.permute(1, 0, 2)),
                      "output/vit_self_atten_v1.onnx",
                      ...
                      do_constant_folding=True)
    LOG.info("export done")
    helper_onnx("output/vit_self_atten_v1.onnx")
    LOG.info("helper_onnx done")
    exit(0)
