...
class TemporalSelfAttention(BaseModule):
    ...
    def forward(self,
        ...
        logger.info(pt("spatial_shapes", spatial_shapes))  #  torch.Size([1, 2])  torch.int64  tensor([[20, 20]])
        logger.info(pt("level_start_index", level_start_index))    # torch.Size([1])  torch.int64   tensor([0])
        logger.info(pt("sampling_locations", sampling_locations))  # torch.Size([2, 400, 8, 1, 4, 2])  torch.float32
        logger.info(pt("attention_weights", attention_weights))    # torch.Size([2, 400, 8, 1, 4])  torch.float32  
        logger.info(pt("self.im2col_step", self.im2col_step))      # <class 'int'>  64
