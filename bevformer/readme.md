...
+ 对于 BEVFormerV2/obtain_history_bev 函数    
  ...
  + 接着进行pts_bbox_head（BEVFormerHead forward with only_bev=True）    
    + 进入 PerceptionTransformerV2 的 get_bev_features （实质是 PerceptionTransformerBEVEncoder 的 forward），返回得到 bev_embed（prev_bev）。
...
+ 对上一步骤得到的 img_feats 以及历史帧的 prev_bev 进行 simple_test_pts(img_feats, img_metas, prev_bev)    
  +（BEVFormerHead）pts_bbox_head 
  +（BEVFormerHead）pts_bbox_head.get_bboxes
