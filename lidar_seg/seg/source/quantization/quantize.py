...
def transfer_torch_to_quantization(nninstance : torch.nn.Module, quantmodule):
    ...
    def __init__(self): 
        try:
          ...
          logger.warning("only quan input, {}".format(self.__class__))
...
def apply_custom_rules_to_quantizer(model : torch.nn.Module, export_onnx : Callable):
    ...
    export_onnx(model, "1126quantization-custom-rules-temp.onnx")
    pairs = find_quantizer_pairs("1126quantization-custom-rules-temp.onnx")
    for major, sub in pairs:
        #logger.info(f"Rules: {sub} match to {major}")
        get_attr_with_path(model, sub)._input_quantizer = get_attr_with_path(model, major)._input_quantizer
    os.remove("1126quantization-custom-rules-temp.onnx")
...
def calibrate_model(model : torch.nn.Module, dataloader, device, num_batch=4):
    ...
    def collect_stats(model, data_loader, device, num_batch=200):
        ...
        with torch.no_grad():
            for i, datas in tqdm(enumerate(data_loader), total=num_batch, desc="Collect stats for calibrating"):
                imgs = datas['input'].to(device, non_blocking=True).float()
                model(Variable(imgs))
                if i >= num_batch:
                    break
    ...
    for method in ["mse", "entropy"]: # just for hist
