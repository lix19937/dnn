...
def _slow_neg_loss(pred, gt):
    ...
    neg_weights = torch.pow(1 - gt[neg_inds], 5)
...
class RegL1RotLoss(nn.Module):
    def __init__(self):
        super(RegL1RotLoss, self).__init__()

    def forward(self, output, mask, ind, target):
        pred = _transpose_and_gather_feat(output, ind)
        # print('-----target rot deg',
        #       torch.atan2(target[:, :, 0], target[:, :, 1]) * 180.0 / 3.1415926)
        mask = mask.unsqueeze(2).expand_as(pred).float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='mean')
        loss = F.l1_loss(pred * mask, target * mask, reduction='sum')
        loss = loss / (mask.sum() + 1e-4)
