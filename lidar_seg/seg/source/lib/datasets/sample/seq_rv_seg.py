...
from .lidar_seg import label_change
...
def get_transform(one_odom, dt):
    ...
    return np.array([[cos_theta, sin_theta, one_odom[0] * dt],
                     ...
                     [0, 0, 1]])
class SeqRvSeg(data.Dataset):
    def __init__(self) -> None:
        super().__init__()
        self.img_scale=(6, 192, 1024)
        fov_angle=(-16, 8)
        horizon_angle=(-64, 64)
    ...
    def project_to_rv(self, merge_cloud, label):
        """Resize images with ``results['scale']``."""
        # ori_cloud = cloud.copy()
        # ori_cloud[:, -1] = 1
        ori_cloud = merge_cloud[:, :3]
        ...
        for key, val in dict_mat.items():
            if self._in_roi(key):
                ...
                img_ori[5, key[0], key[1]] = merge_cloud[:, -1][val[-1]]
    ...
    def __getitem__(self, index):
        ...
        stamp = float(data_path.split('/')[-1][:20])
        ...
        odom_path = data_path.replace('pcd_dir', 'last_odom_dir')
        odom_data = np.loadtxt(odom_path)
        if odom_data[-1] == 1:
            last_data_path = data_path.replace('pcd_dir', 'last_pcd_dir')
            lidar_last_frame = np.fromfile(last_data_path, np.float32).reshape(-1, 4)
            lidar_last_frame[:, -1] = 1
            last_stamp = float(odom_data[0])
            tf = get_transform(odom_data[1:], stamp - last_stamp)
            tmp = np.ones((lidar_last_frame.shape[0], 3), np.float32)
            tmp[:, :2] = lidar_last_frame[:, :2]
            lidar_last_frame[:, :2] = (tf @ tmp.T).T[:, :2]
            lidar_one_frame = np.vstack((lidar_one_frame, lidar_last_frame))
            gt_seg_label = np.hstack((gt_seg_label,
            np.ones((1, lidar_last_frame.shape[0]), np.uint8) * self.opt.ignore_index))
