...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.quantize = True # False # True   lixxxxyyyy
    ...
    def get_model(self, head_type='saic'):
        ...
        if head_type == 'saic':
            ...
            from yolox.models import YOLOXHead as Head

        logger.info(" =======  get_model ========= quantize:{}".format(self.quantize))    
        def init_yolo(M):
            for m in M.modules():
        ...
        if getattr(self, "model", None) is None:
            ...
            backbone = YOLOPAFPN(self.depth, self.width, in_channels=in_channels, act=self.act, quantize=self.quantize)
            head = Head(self.num_classes, self.width, in_channels=in_channels, act=self.act, quantize=self.quantize)
            self.model = YOLOX(backbone, head, quantize=self.quantize)
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, cache_img=False):
        ...
        with wait_for_the_master():
            dataset = COCODataset(data_dir=self.data_dir,
    ...
    def get_lr_scheduler(self, lr, iters_per_epoch):
        ...
        scheduler = LRScheduler(self.scheduler,
                                lr,
                                iters_per_epoch,
                                self.max_epoch,
