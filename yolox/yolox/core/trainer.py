...
from yolox.utils import (MeterBuffer, ModelEMA, WandbLogger, adjust_status, all_reduce_norm, get_local_rank, get_model_info, get_rank, get_world_size, gpu_mem_usage, is_parallel, load_ckpt,
                         occupy_mem, save_checkpoint, setup_logger, synchronize)
...
class Trainer:
    def __init__(self, exp: Exp, args):
        ...
        # add for quantization lixxxx
        self.per_channel_quantization = self.exp.quantize
    ...
    def train(self):
        logger.info('=====> device_count:{}'.format(torch.cuda.device_count()))
        logger.info('=====> is_available:{}'.format(torch.cuda.is_available()))
    ...
    def train_in_epoch(self):
        logger.info("========> start_epoch:{}, max_epoch:{}".format(self.start_epoch, self.max_epoch))
    ...
    def before_train(self):
        ...
        # model related init
        logger.info("local_rank:{}".format(self.local_rank))
        torch.cuda.set_device(self.local_rank)
        model = self.exp.get_model(head_type=self.exp.head_type)
        logger.info('model.training: {}'.format(model.training))
		
        if self.per_channel_quantization:
          logger.info('=========== NV-quantization=============')
        ...
        self.evaluator = self.exp.get_evaluator(data_dir=self.exp.data_dir, batch_size=self.args.batch_size, is_distributed=self.is_distributed)
    ...
    def after_epoch(self):
        ...
        ckpt_name = 'epoch_%03d' % self.epoch
        self.save_ckpt(ckpt_name=ckpt_name)
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            logger.info(
                "{}, mem: {:.0f}Mb, {}, {}, lr: {:.3e}".format(progress_str, gpu_mem_usage(), time_str, loss_str, self.meter["lr"].latest, ) + (", size: {:d}, {}".format(self.input_size[0], eta_str)))
    ...
    def resume_train(self, model):
        if self.args.resume:
            ...
            if self.args.ckpt is not None:
                ...
                ckpt_file = self.args.ckpt
                ckpt = torch.load(ckpt_file, map_location=self.device)
                logger.info("ckpt {}".format(type(ckpt)))                
    ...
    def evaluate_and_save_model(self):
        ...
        with adjust_status(evalmodel, training=False):
            (ap50_95, ap50, summary), predictions = self.exp.eval(evalmodel, self.evaluator, self.is_distributed, return_outputs=True)
        ...
        if self.rank == 0:
            ...
            if self.args.logger == "wandb":
                self.wandb_logger.log_metrics({"val/COCOAP50": ap50, "val/COCOAP50_95": ap50_95, "train/epoch": self.epoch + 1, })
    ...
    def save_ckpt(self, ckpt_name, update_best_ckpt=False, ap=None):
        if self.rank == 0:
            ...
            if self.args.logger == "wandb":
                self.wandb_logger.save_checkpoint(self.file_name,
                                                  ckpt_name,
                                                  update_best_ckpt,
                                                  metadata={"epoch": self.epoch + 1,
                                                            "optimizer": self.optimizer.state_dict(),
                                                            "best_ap": self.best_ap,
                                                            "curr_ap": ap}
                                                  )
