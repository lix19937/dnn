...
def poly2obb_np_batch(polys_batch, version='oc'):
    assert polys_batch.shape[-1] == 8, 'the last dim of polys_batch should be 8 but got {}'.format(polys_batch.size()[-1])
    results = np.empty([polys_batch.shape[0], 5])
    for index, polys in enumerate(polys_batch.astype(np.float32)):
        obb = poly2obb_np(polys, version)
        results[index][:] = obb
    return results
...
# def obb2poly_np_batch(rbboxes_batch, version='oc'):
#     assert rbboxes_batch.shape[-1] == 5, 'the last dim of rbboxes_batch should be 5, but got {}'.format(rbboxes_batch.shape[-1])
#     results = np.empty([rbboxes_batch.shape[0], 8])
#     for index, rbboxes in enumerate(rbboxes_batch):
#         polys = obb2poly_np(rbboxes, version)
#         results[index][:] = polys
#
...
def obb2poly_np_pure_oc(rbboxes):
    """Convert oriented bounding boxes to polygons.
