...
def main():
    ...
    if not args.no_onnxsim:
        ...
        if args.platform == 'dla':
            ...
            nodes = graph.node
            vec = ['Concat_164', 'Concat_177', 'Concat_190']
            vec_o = ['o1', 'o2', 'o3']
            k = 0
            for i in range(len(nodes)):
                if nodes[i].name.find("Concat_") != -1:
                  print("idx:{}, {}".format(i, nodes[i]))

                if nodes[i].name in vec:
                  print("idx:{}, {}".format(i, nodes[i]))
                  nodes[i].output[0] = vec_o[k]
                  k = k + 1
                  print("===:{}, {}".format(i, nodes[i]))

            i = len(nodes) - 1
            graph.node.remove(nodes[i])
            graph.node.remove(nodes[i-1])
            graph.node.remove(nodes[i-2])
            graph.node.remove(nodes[i-3])
            graph.node.remove(nodes[i-4])
            
            del model_simp.graph.output[:]

            info0 = onnx.helper.make_tensor_value_info('o1', onnx.TensorProto.FLOAT, [1, 6, 80, 80])
            info1 = onnx.helper.make_tensor_value_info('o2', onnx.TensorProto.FLOAT, [1, 6, 40, 40])
            info2 = onnx.helper.make_tensor_value_info('o3', onnx.TensorProto.FLOAT, [1, 6, 20, 20])           

            model_simp.graph.output.append(info0)
            model_simp.graph.output.append(info1)
            model_simp.graph.output.append(info2)                
