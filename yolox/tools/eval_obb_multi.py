...
def main(exp, args, num_gpu):
    ...
    best_ap = dict(file=None, ap50=0)
    for index, ckpt_file in enumerate(sorted(glob.glob(args.ckpt + '/*.pth'))):
        # if index < 100:
        #     continue
        logger.info('------------------ {}:{} -------------------'.format(index, ckpt_file))
        model = exp.get_model(head_type='obb')
        evaluator = exp.get_evaluator(args.data_dir, args.batch_size, is_distributed, args.test, args.legacy)
        ...
        # start evaluate
        mAPs, _, _ = evaluator.evaluate(model, args.fp16, is_distributed, decoder=decoder, trt_file=trt_file, test_size=exp.test_size)
        logger.info('maps:{}'.format(mAPs))
        if mAPs[0] > best_ap.get('ap50'):
            best_ap['file'] = ckpt_file
            best_ap['ap50'] = mAPs[0]
        logger.info('best_ap:{}'.format(best_ap))
