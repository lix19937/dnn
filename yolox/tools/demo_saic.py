...
def make_parser():
    ...
    parser.add_argument("-p", "--plan", type=str, default=None)
...
class Predictor(object):
    def __init__(self, model, exp, cls_names=_CLASSES, trt_file=None, decoder=None, device="gpu", fp16=False):
        ...
        self.device = device
        self.fp16 = fp16
        self.mean_std = exp.mean_std
        self.preproc = ValTransform(self.mean_std)
        ...
        self.engine = EngineFromBytes(BytesFromPath(trt_file))
    ...
    def inference(self, img):
        ...
        with torch.no_grad():
            ...
            with TrtRunner(self.engine) as runner:
              feed_dict = {"images": img.reshape(1, 3, 640, 640).cpu().numpy()}
              output = runner.infer(feed_dict)["output"] 
              outputs = torch.from_numpy(output)
            if self.decoder is not None:
                outputs = self.decoder(outputs, dtype=outputs.type())

            outputs = obbpostprocess(outputs, self.num_classes, self.confthre, self.nmsthre)
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        # cls = output[:, 10]
        cls = np.zeros_like(output[:, 8])
        scores = output[:, 8]

        vis_res = obb_vis(img, rbboxes, scores, cls, cls_conf, self.cls_names)
        return vis_res


def image_demo(predictor, vis_folder, path, current_time, save_result, idx=None):
...
def main(exp, args):
    ...
    predictor = Predictor(model, exp, trt_file=trt_file, decoder=decoder)
